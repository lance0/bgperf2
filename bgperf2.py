...
def bench(args):
    ...
    tracker = ConvergenceTracker(
        no_progress_deadline_seconds=zero_route_failure_grace_seconds(args.target),
        preserve_assurance_after_neighbors_checkpoint=(args.target == 'rustbgpd'),
    )
    while True:
        info = q.get()
        ...
        if info['who'] == m.name:
            ...
            if monitor_confirms_neighbor_checkpoint(
                    args.target, recved, output_stats['required'],
                    tracker.neighbors_checkpoint):
                tracker.note_neighbors_checkpoint()
