...
class FixtureTester(BaseTester):
...
class FakeDocker:
    ...
    def exec_start(self, exec_id, **kwargs):
        return b'2: eth0    inet 10.0.0.2/24 scope global eth0\n'
...
class TesterProvenanceTests(unittest.TestCase):
    ...
    def test_bird_expected_messages_are_not_errors(self):
        ...
        self.write_log(
            ...
            '\n'.join((
                ...
                'RMT Invalid route 192.0.2.0/24 withdrawn',
    ...
    def assert_bench_repeat_rejected_before_mutation(self, value):
        ...
        args = SimpleNamespace(
            repeat=value, dir=str(run_root), bench_name='row', file=None,
            target='bird', version=None, image='fixture:image',
    ...
    def test_diagnostic_methods_are_instances_without_shell_or_global_tmp(self):
        for tester_class in (BaseTester, BIRDTester, GoBGPMRTTester, Bgpdump2Tester):
            with self.subTest(tester_class=tester_class.__name__):
                self.assertEqual(
                    ['self'],
